...
public class MEMSqlEngine {
	...
	public Message createDatabase(String dbName) {
		dbName = imdsUtils.convertToUpperCase(imdsUtils.trimString(dbName));
	...
	public Message createTable(String dbName, String tableName,
		...
		dbName = imdsUtils.convertToUpperCase(dbName);
		tableName = imdsUtils.convertToUpperCase(tableName);
		columnsDefinition = imdsUtils.convertToUpperCase(columnsDefinition);
		...
		if (database != null) {
			...
			Message m = (new imdsParser()).parseColumnDefinition(
	...
	public Message insertStatement(String dbName, String tableName,
		...
		if (database != null) {
			if (database.tables != null) {
				...
				if (table != null) {
					return ((new imdsParser()).addRow(table, values));
	...
	public Message selectStatement(String dbName, String tableName,
		...
		dbName = imdsUtils.convertToUpperCase(imdsUtils.trimString(dbName));
		tableName = imdsUtils.convertToUpperCase(imdsUtils
				.trimString(tableName));
		columnNames = imdsUtils.convertToUpperCase(imdsUtils
		...
		if (database != null) {
			if (database.tables != null) {
				...
				if (table != null) {
					...
					if (!m.error) {
						m = (new imdsParser()).calculateAggregates(table,
								columnNames);
						return m;
					}
					...
					if (!m.error) {
						...
						m = (new imdsParser()).selectTable(table, columnNames,
								filterCondition);
